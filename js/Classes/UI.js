import {deleteAppointment, editAppointment, DB} from "../functions.js"
...
class UI {
    ...
    showAppointmentsHTML() {
        ...
        const objectStore = DB.transaction("AppointmentsTable").objectStore("AppointmentsTable");

        //Cursor iterate over all the elements in the Database.
        objectStore.openCursor().onsuccess = function(e) {
            const cursor = e.target.result;
            if(cursor) {
                const {pet, owner, tel, date, hour, symptom, id} = cursor.value;
                const divAppointment = document.createElement("div");
                divAppointment.classList.add("cita", "p-3");
                divAppointment.dataset.id = id;
                ...
                const cursorValueFixed = cursor.value; //this storage the cursor object so it prevents continue iterating
                
                editBtn.onclick = () => editAppointment(cursorValueFixed);
                ...
                //after iterate over the first element, this allow to go to the next element
                cursor.continue();
            }
        }
    }
